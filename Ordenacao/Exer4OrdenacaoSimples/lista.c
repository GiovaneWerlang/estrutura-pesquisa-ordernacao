...
void ordena_selecao(Lista *l){
    ...
    for(aux = l->primeira; aux->prox !=NULL; aux = aux->prox){
        min = aux;
        for(auxJ = aux->prox; auxJ !=NULL; auxJ = auxJ->prox){
            if(auxJ->item.codigo < min->item.codigo){
                min = auxJ;
            }
        }
        troca(l,min->item.codigo, aux->item.codigo);
    }
}

void troca(Lista *l, int min, int i){
    int copo;
    Celula *mini = busca_celula_por_codigo(l, min);
    Celula *indi = busca_celula_por_codigo(l, i);

    copo = mini->item.codigo;
    mini->item.codigo = indi->item.codigo;
    indi->item.codigo = copo;
