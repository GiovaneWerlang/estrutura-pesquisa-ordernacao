...
void distribui_pares_impares(Lista *l, Lista *m, Lista *k){
    Celula *aux;
    Celula *aux2 = m->primeira;
    Celula *aux3 = k->primeira;
    for(aux= l->primeira; aux!=NULL; aux = aux->prox){
        if(aux->item.codigo %2==0){
            adiciona_fim(m,aux->item.codigo);
        }else{
            adiciona_fim(k,aux->item.codigo);
            //aux3->item.codigo = aux;
        }
    }
